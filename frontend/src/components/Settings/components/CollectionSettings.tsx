...
export default function CollectionSettings() {
  ...
  const {
    showMaCollection, setShowMaCollection,
  ...
  const {
    traducteurs, traducteursLoading, fetchTraducteurs,
  ...
  // ── État enrichissement synopsis collection utilisateur ──────────────────
...
const handleCollectionSynopsis = useCallback(async () => {
  ...
  return (
    ...
    <div className="settings-grid">
      ...
      <section className="settings-section">
        <h4 className="settings-section__title">📝 Traduction synopsis (Ma collection)</h4>
        <p className="settings-log-description">
          Lance la récupération et la traduction EN→FR des synopsis pour les jeux de votre collection
          liés à la base F95. Utile après un import depuis le script Tampermonkey ou une autre source.
        </p>
        ...
        <div className="f95-enrich-scrape-section">
          <label className="f95-lib-import-toggle f95-enrich-toggle-main">
            <input
              ...
              onChange={e => setCollSynopsisForce(e.target.checked)}
              disabled={collSynopsisRunning}
            />
            <span>
              🔄 Forcer la re-traduction
              <em> (même pour les jeux qui ont déjà un synopsis FR)</em>
            </span>
          </label>
        </div>
          ...
          {!collSynopsisSummary ? (
            !collSynopsisRunning ? (
              <button
                ...
                disabled={!user}
              >
                🚀 Lancer la traduction des synopsis
              </button>
            ) : (
              <>
      ...
      <section className="settings-section">
        ...
        <div className="f95-enrich-scrape-section">
          <label className="f95-lib-import-toggle f95-enrich-toggle-main">
            <input type="checkbox" checked={enrichScrapeMissing} onChange={e => setEnrichScrapeMissing(e.target.checked)} disabled={enrichRunning} />
            <span>
      ...
      <section className="settings-section">
        ...
        <div className="collection-f95-select-row">
          <select className="app-input collection-f95-select" value={selectedTraducteur} onChange={e => setSelectedTraducteur(e.target.value)} disabled={traducteursLoading}>
            <option value="">— Choisir un traducteur —</option>
            {traducteurs.map(t => <option key={t} value={t}>{t}</option>)}
          </select>
          <button type="button" className="form-btn form-btn--primary" onClick={() => selectedTraducteur && fetchPreview(selectedTraducteur)} disabled={!selectedTraducteur || previewLoading}>
        ...
        {preview && f95Items.length > 0 && (
          <>
            <div className="collection-import-preview">
              <div className="collection-import-preview-stat"><span className="collection-import-stat-value">{preview.count}</span><span className="collection-import-stat-label">jeux</span></div>
              <div className="collection-import-preview-stat"><span className="collection-import-stat-value">{preview.new_count}</span><span className="collection-import-stat-label">nouveaux</span></div>
              <div className="collection-import-preview-stat"><span className="collection-import-stat-value">{preview.already_in_collection}</span><span className="collection-import-stat-label">déjà en collection</span></div>
            </div>

            <div className="collection-import-options">
              <Toggle label="Ignorer les jeux déjà en collection (recommandé)" checked={f95SkipExisting && !f95OverwriteAll} onChange={v => { setF95SkipExisting(v); if (v) setF95OverwriteAll(false); }} disabled={f95Importing || f95OverwriteAll} size="sm" />
              <Toggle label="Écraser les données des jeux déjà en collection" checked={f95OverwriteAll} onChange={v => { setF95OverwriteAll(v); if (v) setF95SkipExisting(false); }} disabled={f95Importing} size="sm" />
            </div>
            ...
            <div className="collection-f95-list styled-scrollbar">
              {f95Items.map(j => {
                ...
                return (
                  <Toggle key={j.site_id} checked={f95SelectedIds.has(j.site_id)} onChange={() => f95ToggleItem(j.site_id)} label={label} size="sm" className="collection-f95-list-item" />
                );
              })}
            </div>
            ...
            {!f95Summary ? (
              <div className="collection-import-actions">
                ...
                {!f95Importing ? (
                  <button type="button" className="form-btn form-btn--primary" onClick={() => f95StartImport(selectedTraducteur, { skipExisting: f95SkipExisting, overwriteAll: f95OverwriteAll }, f95SelectedIds.size > 0 ? [...f95SelectedIds] : undefined)} disabled={f95SelectedIds.size === 0}>
