...
export default function ErrorModal({ error, onClose, onRetry }: ErrorModalProps) {
  ...
  const getSuggestions = (): string[] => {
    ...
    if (status === 401 || status === 403) {
      ...
      suggestions.push('Consultez status.discord.com pour plus d\'informations');
    } else if (!status || status === 0 || error.message.includes('fetch') || error.message.includes('network')) {
      ...
      suggestions.push('Consultez la console (Ctrl+Shift+I) pour voir les logs de démarrage');
      ...
      suggestions.push('Configurez l\'URL de l\'API dans Configuration');
    }
