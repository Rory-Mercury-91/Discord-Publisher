...
export type ScrapedData = {
  ...
  /** Date de dernière mise à jour du thread sur F95Zone (YYYY-MM-DD) */
...
export type ManualGameData = {
  ...
  synopsis?       : string | null;
  /** Synopsis EN explicite */
  synopsis_en?    : string | null;
  /** Synopsis FR (traduction française) */
  synopsis_fr?    : string | null;
};
        ...
        if (jeux?.length) {
          ...
          for (const [key, group] of byKey) {
            ...
            const gamePayload = {
              ...
              f95_date_maj      : primary.f95_date_maj ?? undefined,
      ...
      setItems(
        rows.map((r) => {
          ...
          if (r.scraped_data) {
            ...
            const synopsisEn = (s.synopsis_en ?? s.synopsis ?? '').trim();
  ...
  const resolveF95 = useCallback(
    async (
      ...
      const body: Record<string, unknown> = isNumeric
        ? { f95_thread_id: parseInt(trimmed, 10) }
        : { url: trimmed };

      const f95Cookies = typeof localStorage !== 'undefined'
